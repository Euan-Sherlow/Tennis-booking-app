...
import re 
from users_details import EMAIL_DOESNT_EXIST, MAX_CHARS, SAFE_CREATE_DELETE, USER_DOESNT_EXISTS, USER_EXISTS, VALID_USER, VALID_ADMIN, WRONG_PASSWORD
...
class Club_Admin_Tests(unittest.TestCase):
    ...
    def test_handle_booking_admin_sucessful(self):
        ...
        with app.test_request_context('/api/handle_booking', method='PUT',content_type='application/json'):
            with self.app.session_transaction() as sess:
                sess['role'] = self.role
                sess['testing'] = True
            load = {"court":"grass_one", "date":"2023-12-11", "time":"13", "user":"jbousfield92@outlook.com", "cancel":False}
    ...
    def test_handle_booking_admin_wrong_court(self):
        ...
        with app.test_request_context('/api/handle_booking', method='PUT',content_type='application/json'):
            with self.app.session_transaction() as sess:
                sess['role'] = self.role
            load = {"court":"grass_three", "date":"2023-11-19", "time":"17", "user":"jbousfield92@outlook.com", "cancel":False}
            response = self.app.put('/api/handle_booking', data=json.dumps(load), content_type="application/json")
            response_object = json.loads(response.data.decode('utf-8'))
            self.assertTrue(response_object['title'] == "Booking Error!")
    ...
    def test_handle_booking_admin_wrong_time(self):
        ...
        with app.test_request_context('/api/handle_booking', method='PUT',content_type='application/json'):
            with self.app.session_transaction() as sess:
                sess['role'] = self.role
            load = {"court":"grass_three", "date":"2023-11-19", "time":"2400", "user":"jbousfield92@outlook.com", "cancel":False}
            response = self.app.put('/api/handle_booking', data=json.dumps(load), content_type="application/json")
            response_object = json.loads(response.data.decode('utf-8'))
            self.assertTrue(response_object['title'] == "Booking Error!")
    ...
    def test_handle_cancellation_admin_sucessful(self):
        ...
        with app.test_request_context('/api/handle_booking', method='PUT',content_type='application/json'):
            with self.app.session_transaction() as sess:
                sess['role'] = self.role
                sess['testing'] = True
            load = {"court":"grass_one", "date":"2023-12-11", "time":"13", "user":"jbousfield92@outlook.com", "cancel":True}
            response = self.app.put('/api/handle_booking', data=json.dumps(load), content_type="application/json")
            response_object = json.loads(response.data.decode('utf-8'))
    ...
    def test_delete_user_admin_successful(self):
        ...
        with app.test_request_context('/admin/delete_account', method='POST',content_type='application/json'):
            with self.app.session_transaction() as sess:
                sess['role'] = self.role
                sess['testing'] = True
                sess['username'] = "jbousfield92@outlook.com"
            loads = [{"delete_pword":"SE-Team12", "email_delete":"createUser@outlook.com"},{"delete_pword":"SE-Team12", "email_delete":"createAdmin@outlook.com"}]
            for load in loads:
    ...
    def test_delete_user_admin_failure(self):
        ...
        with app.test_request_context('/admin/delete_account', method='POST',content_type='application/json'):
            with self.app.session_transaction() as sess:
                ...
                sess['testing'] = True
                sess['username'] = "jbousfield92@outlook.com"
            loads = [{"delete_pword":"SE-Team12", "email_delete":"create.com"},{"delete_pword":"SE-Team12", "email_delete":"createAdmin.com"}]
            for load in loads:
                response = self.app.post('/admin/delete_account', data=load)
